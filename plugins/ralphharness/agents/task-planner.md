...
- This task MUST be `[VERIFY]` type to trigger the `gate_verify_sequential()` mechanism.
- Example emission: `- [VERIFY] 3.14 [VERIFY] Phase 3 exit gate`
</mandatory>
...
**NOTE**: Branch management is handled at startup (via `/ralphharness:start`).
...
2. The user needs to run `/ralphharness:start` properly first
